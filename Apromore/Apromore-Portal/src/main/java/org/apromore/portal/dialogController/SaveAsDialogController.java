...
//    private Textbox new_branch_name_of_model1_to_be_evolved;
    ...
    private Boolean isPropagateChange_fromCGtoG = false;
...
//        /*add the label and radio group, in the 4th rows*/
        this.label2 = (Label) rows.getChildren().get(4).getChildren().get(0);
        this.radiogroup2 = (Radiogroup) rows.getChildren().get(4).getChildren().get(1);
        this.radio3 = (Radio) radiogroup2.getChildren().get(0);
        this.radio4 = (Radio) radiogroup2.getChildren().get(1);

        radio4.setChecked(true);

        radio3.addEventListener("onClick",
                new EventListener<Event>() {
                    public void onEvent(Event event) throws Exception {
                        setIsPropagateChange_fromCGtoG(true);
                    }
                });

        radio4.addEventListener("onClick",
                new EventListener<Event>() {
                    public void onEvent(Event event) throws Exception {
                        setIsPropagateChange_fromCGtoG(false);
                    }
                });

        /*add the button group in the 5th row*/
        Row buttonGroupR = (Row) rows.getChildren().get(5);
        ...
        try {
            ...
            if (validateFields()) {
                if (!isNormalSave) {
                    ...
                    if(!isPropagateChange && !isPropagateChange_fromCGtoG ){
                    ...
                    /*Case2: consider change propagation from G to CG*/
                    else if (isPropagateChange && !isPropagateChange_fromCGtoG){
                    ...
                    /*Case3: consider change propagation from CG to G*/
                    else if (!isPropagateChange && isPropagateChange_fromCGtoG){

                        getService().updateProcess(editSession.hashCode(), userName, nativeType, processId, domain, process.getName(),
                                editSession.getOriginalBranchName(), branch, versionNo, originalVersionNumber, versionName, is,2);
                    }
...
//    }
    ...
    public void setIsPropagateChange_fromCGtoG (Boolean isPropagateChange_fromCGtoG){

        this.isPropagateChange_fromCGtoG = isPropagateChange_fromCGtoG;
    }
